...
log "Starting cleaning up remotes on `date`"                                   2>&1 | tee -a ${logfile}
loop_over_all_repos "Cleaning up remotes" "${cmd_cleanup_remotes}"
log "Done cleaning up remotes on `date`"                                       2>&1 | tee -a ${logfile}
